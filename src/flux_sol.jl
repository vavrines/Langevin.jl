        ...
        @inbounds @threads for i in eachindex(flux.fw)
            for j in axes(sol.w[1], 2) # over gPC coefficients or quadrature points
                ...
                KitBase.flux_kfvs!(
                    ...
                    fh,
                    fb,
                    sol.h[i-1][:, j] .+ 0.5 .* KS.ps.dx[i-1] .* sol.sh[i-1][:, j],
                    sol.b[i-1][:, j] .+ 0.5 .* KS.ps.dx[i-1] .* sol.sb[i-1][:, j],
                    sol.h[i][:, j] .- 0.5 .* KS.ps.dx[i] .* sol.sh[i][:, j],
                    sol.b[i][:, j] .- 0.5 .* KS.ps.dx[i] .* sol.sb[i][:, j],
                    KS.vs.u,
                    KS.vs.weights,
                    dt,
                    sol.∇h[i-1][:, j],
